...
function PANEL:Init()
	...
	self.containerPanels["lineages"] = self.backgroundList
	self.containerPanels["feats"] = self.backgroundPanel
	-- attributes subpanel
	...
	if not self.bInitialPopulate then
		...
		for _, v in SortedPairs(ix.faction.teams) do
			if ix.faction.HasWhitelist(v.index) then
				...
				if (lastSelected and lastSelected == v.index) or (not lastSelected and v.isDefault) then
