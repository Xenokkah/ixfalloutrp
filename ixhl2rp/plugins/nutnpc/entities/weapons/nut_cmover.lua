-- "gamemodes\\halorp\\plugins\\combat\\entities\\weapons\\nut_cmover.lua"
...
function SWEP:DrawHUD()
	if(CLIENT) then
		...
		if(self:GetNetVar("selected")) then
			...
			ix.util.DrawText(name.. " Selected", ScrW() * 0.5, ScrH() * 0.3, ColorAlpha(Color(50,50,255), alpha), 1, 1, "ixSmallFont", alpha * 1)
			ix.util.DrawText("Primary Fire to move selected to location.", ScrW() * 0.5, ScrH() * 0.3 + 16, ColorAlpha(Color(50,255,50), alpha), 1, 1, "ixSmallFont", alpha * 1)
		end
		...
		if(self.viewed) then
			...
			ix.util.DrawText(name.. " Viewed", ScrW() * 0.5, ScrH() * 0.7, ColorAlpha(Color(255,50,50), alpha), 1, 1, "ixSmallFont", alpha * 1)
		end
	end
end
