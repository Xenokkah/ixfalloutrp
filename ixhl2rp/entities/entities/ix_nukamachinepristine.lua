...
ENT.Type             = "anim"
ENT.PrintName        = "Pristine Nuka-Cola Vending Machine"
ENT.Author            = "Scrat"
...
function ENT:Use(activator)
    if (activator:IsPlayer()) then
        ...
        if (target:GetInventory():HasItem("prewarmoney")) then
            ...
            activator:NewVegasNotify("You deposit dollar bills, and the machine dispenses an ice cold bottle immediately.", "messageNeutral", 5)
            activator:EmitSound("fosounds/fix/ui_items_bottlecaps_up_03.mp3")
        
            self:SetVar("Stock", self:GetVar("Stock") - 1)
            target:GetInventory():Add("nukacolacold", 1)
...
if (CLIENT) then
    function ENT:OnPopulateEntityInfo(tooltip)
        ...
        descriptionfull:SetText("A clean and fully-functional Nuka-Cola machine with bottles still inside. Stocked by the main flavor only, but guaranteed ice cold. Uses pre-war money." )
